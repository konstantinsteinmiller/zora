...
<template lang="pug">
  div.top-0.left-0.card.glass.dark(v-if="$.isFairyCollection" class="!fixed z-[10] w-[99.5vw] h-[99.5vh] translate-x-[0.25vw] translate-y-[0.25vh] !rounded-[6px] !border-x-0")
    div.collection.flex.gap-2.h-full.mt-16.relative
      div.preview.mr-8.p-4.px-6(class="min-w-[298px]")
        div.relative.overflow-visible(style="background: rgba(0, 0, 0, 0.0);" class="min-h-[300px] max-w-[250px]")
          div.name.relative.p-1.h-8.flex.items-center.justify-center.overflow-visible(class="!border-0 bg-[#151515]")
            ...
            img.frame.absolute.bottom-0.left-0(src="/images/frames/hr.png" :alt="`horizontal-line`" class="!scale-[98%] h-[9px] -mb-[8px] z-10")
          div.preview-img.relative(class="")
            img.frame.absolute.top-0.left-0(src="/images/frames/fancy-frame_512x512.png" :alt="`frame`" class="scale-[110%] h-[326px] -mt-[35px]")
          ...
          div.number.mt-12.text-lg.mb-1.flex.justify-center.items-center(
          ...
          div.stats.mt-4.text-lg.rounded-xl.mb-1(v-if="selectedFairy?.statsGrowthVisual" class="")
            ...
            div(class="bg-[#acacac] mb-[2px]")
              div.grid.grid-cols-2(v-for="stat in statsVisualsList" :key="selectedFairy.id + stat.key" class="mb-[2px]")
                ...
                div.px-2.bg-white.flex.items-center(class="text-[#454545] py-[1px]")
                  div.rounded-full.flex.items-center(v-if="stat.key === 'type'" class="bg-[#454545] p-[2px] px-1")
      ...
      div.list.card.glass.frame.gap-4.flex.items-start.justify-start.p-4.mr-4.mb-40.w-full(
      ...
      img.absolute.right-6(src="/images/logo/Zora_logo_300x246.png" alt="logo"
        class="w-[100px] bottom-[72px]"
      )
</template>

<style scoped lang="sass">
.collection
  background: radial-gradient(800px circle at 45vw 55vh, rgba(95, 21, 198, 0.09) 30%,  transparent 55%) !important
.rib
  & *, &
