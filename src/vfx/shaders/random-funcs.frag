...
void main() {
   ...
   // uv = value_noise(-uv*3.0+ mod(u_time,71.0)*0.5,seed,5057.0,3505.0);
    //uv += value_noise(uv*64.5,seed+1.0,12.0,5257.0)*0.02;
    //uv += value_noise(-uv*3.0+ mod(u_time,71.0)*0.5,seed,5057.0,3505.0)*0.06;
    
	float v_dist = voronoi_dist(uv,51.0+u_time*0.1,57.0+u_time*0.1)*4.0;
    
    //float n = value_noise(uv*4.5+u_time*0.1,seed+3.0,0.0,57.0)*2.0;
    
    vec3 color = vec3(0.4,0.8,1)*(v_dist*v_dist);   
